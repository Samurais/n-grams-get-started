...
if [ ! -d liblbfgs-1.10 ]; then
    ...
    bash $baseDir/install_liblbfgs.sh || exit 1
fi

# http://www.speech.sri.com/projects/srilm/download.html
if [ ! -f $baseDir/../downloads/srilm-1.7.0.tgz ]; then
  ...
  echo put it in ./srilm-1.7.0.tgz, then run this script.
  exit 1
fi
...
mkdir -p srilm
cd srilm
tar -xvzf $baseDir/../downloads/srilm-1.7.0.tgz
